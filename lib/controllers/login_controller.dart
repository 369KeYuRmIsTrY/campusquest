...
class LoginController extends ChangeNotifier {
      ...
      if (session != null) {
        ...
        if (userId != null) {
          ...
          if (response != null) {
            ...
            if (_role == 'student') {
              await _restoreStudentData();
            } else if (_role == 'instructor') {
              await _restoreInstructorData();
            }
          }
        }
        notifyListeners();
  ...
  Future<void> _restoreStudentData() async {
    try {
      final studentData = await _supabase
          .from('student')
          .select('student_id, name, dept_name, program_id, current_semester')
          .eq('user_id', int.parse(userId!))
          .single();
      ...
      if (studentData != null) {
        ...
        await prefs.setInt('studentId', _studentId!);
        await prefs.setString('studentName', _studentName!);
        if (_deptName != null) await prefs.setString('deptName', _deptName!);
        await prefs.setInt('programId', _programId!);
        await prefs.setInt('currentSemester', _currentSemester!);
      }
    } catch (e) {
  ...
  Future<void> _restoreInstructorData() async {
    try {
      final instructorData = await _supabase
          .from('instructor')
          .select('instructor_id, name, designation')
      ...
      print("Error restoring instructor data: $e");
    }
  }
}
