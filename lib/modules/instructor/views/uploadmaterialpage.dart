...
class _UploadMaterialsPageState extends State<UploadMaterialsPage>
  ...
  void initState() {
    ...
    _uploadDateController.text = DateFormat(
      'yyyy-MM-dd',
    ).format(DateTime.now());
  }
  ...
  Future<void> _initializeData() async {
    ...
    try {
      ...
      if (_instructorId != null) {
        await Future.wait([_fetchCourses(), _fetchNotes()]);
  ...
  Future<void> _addOrUpdateNote() async {
    ...
    try {
      final noteData = {
        'course_id':
            selectedCourseIds
                .first, // Only the first selected course is used here
  ...
  Future<void> _uploadFile() async {
    ...
    try {
      ...
      final fileName =
          '$_instructorId/${DateTime.now().millisecondsSinceEpoch}-${_pickedFile!.name}';
  ...
  void _showAddEditDialog({Map<String, dynamic>? note}) {
    ...
    final titleController = TextEditingController(
      text: isEditing ? note!['title'] : _titleController.text,
    );
    final descriptionController = TextEditingController(
      text: isEditing ? note!['description'] : _descriptionController.text,
    );
    final uploadDateController = TextEditingController(
      text: isEditing ? note!['upload_date'] : _uploadDateController.text,
    );
    List<String> dialogSelectedCourses = List.from(
      isEditing ? selectedCourseIds : selectedCourseIds,
    );
                      ...
                      children: [
                        ...
                        _buildTextField(
                          ...
                          onTap: () async {
                            ...
                            if (pickedDate != null)
                              uploadDateController.text = DateFormat(
                                'yyyy-MM-dd',
                              ).format(pickedDate);
                          },
                        ),
                                    ...
                                    courses.map((course) {
                                      ...
                                      return CheckboxListTile(
                                        ...
                                        onChanged: (bool? value) {
                                          setDialogState(() {
                                            if (value == true) {
                                              dialogSelectedCourses.add(
                                                courseId,
                                              );
                                            } else {
                                              dialogSelectedCourses.remove(
                                                courseId,
                                              );
                                            }
                                          });
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Expanded(
                                  child: Text('File: $_uploadedFileName'),
                                ),
                              ],
                            ),
                          ),
                    ...
                    ElevatedButton.icon(
                      ...
                      onPressed: () async {
                        if (titleController.text.isEmpty ||
                            uploadDateController.text.isEmpty ||
                            dialogSelectedCourses.isEmpty) {
                          _showErrorMessage(
                            'Title, date, and at least one course are required',
                          );
                          return;
                        }
                ...
                (_, controller) => Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      Text(
                        'Upload Date: ${DateFormat('MMMM dd, yyyy').format(DateTime.parse(note['upload_date']))}',
                      ),
                      const SizedBox(height: 12),
                      if (note['file_path'] != null)
                        InkWell(
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            onPressed:
                                () => _showDeleteConfirmation(note['note_id']),
