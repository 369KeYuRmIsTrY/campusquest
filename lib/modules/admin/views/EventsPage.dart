...
class _AddEventPageState extends State<AddEventPage>
  ...
  void _performSearch() {
    ...
    setState(() {
      ...
      events =
          originalEvents.where((event) {
            return (event['title']?.toString().toLowerCase() ?? '').contains(
                  searchText,
                ) ||
  ...
  Future<void> _insertEventPrograms(int eventId) async {
    if (selectedProgramIds.isNotEmpty) {
      final insertData =
          selectedProgramIds
              .map(
                (programId) => {'event_id': eventId, 'program_id': programId},
              )
              .toList();
  ...
  Future<void> _uploadFile() async {
    ...
    try {
      ...
      final fileName =
          '${DateTime.now().millisecondsSinceEpoch}-${_pickedFile!.name}';
  ...
  Future<void> _fetchAssociatedPrograms(int eventId) async {
    ...
    setState(() {
      selectedProgramIds =
          response.map((item) => item['program_id'].toString()).toList();
    });
  }
  ...
  void _showAddEditDialog({Map<String, dynamic>? event}) {
    ...
    final titleController = TextEditingController(
      text: isEditing ? event!['title'] : _titleController.text,
    );
    final descriptionController = TextEditingController(
      text: isEditing ? event!['description'] : _descriptionController.text,
    );
    final eventDateController = TextEditingController(
      text: isEditing ? event!['event_date'] : _eventDateController.text,
    );
                  ...
                  builder: (context, setDialogState) {
                    return Container(
                      ...
                      child: Column(
                        ...
                        crossAxisAlignment:
                            CrossAxisAlignment.stretch, // Stretch form fields
                        children: [
                                        ...
                                        programs.map((program) {
                                          ...
                                          return FilterChip(
                                            ...
                                            onSelected: (selected) {
                                              setDialogState(() {
                                                if (selected) {
                                                  dialogSelectedPrograms.add(
                                                    programId,
                                                  );
                                                } else {
                                                  dialogSelectedPrograms.remove(
                                                    programId,
                                                  );
                                                }
                                              });
